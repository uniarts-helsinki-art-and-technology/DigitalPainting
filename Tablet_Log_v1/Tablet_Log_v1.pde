...
void draw() {
  ...
  if (recording) {
    logFile.println(frameCount + ", " + posX + ", " + posY +  ", " + pressure  + ", " + timeinms);
  } else {
  }
  ...
  textAlign(CENTER);
  if (recording) {
    text("recording now participant " + idParticipant + " on log file" + logStartTime + " frame count " + frameCount, width * 0.5, height * 0.8);
  } else {
...
void keyPressed() { // Press a key to save the data
  ...
  case 'n': 
    ...
    logStartTime = "log-" + idParticipant + "_" + year() + "_" + month() + "_" + day() + "_" + hour() + "_" + minute() + "_" + second();
